...
@click.option('--organization', required=True,
              help='Organization name or ID resolved by organization_show.')
...
def reindex_organization_proxy(organization, dry_run, apply_changes,
    ...
    organization_dict = tk.get_action('organization_show')(
        {'ignore_auth': True}, {'id': organization})
    organization_id = organization_dict['id']
    organization_name = organization_dict.get('name') or organization
    expected_proxy = _expected_proxy(organization_dict, organization_name)
